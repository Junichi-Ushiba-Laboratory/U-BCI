#include "recording.h"
//#include "conversions.h"

#include <set>
#include <sstream>
#ifdef XDFZ_SUPPORT
#include <boost/algorithm/string/predicate.hpp>
#include <boost/iostreams/device/file_descriptor.hpp>
#include <boost/iostreams/filter/zlib.hpp>
#endif

#include <QtDebug>
#include <QElapsedTimer>

// Thread utilities
using Clock = std::chrono::high_resolution_clock;

/**
 * @brief try_join_once		joins and deconstructs the thread if possible
 * @param thread			unique_ptr to a std::tread. Will be reset on success
 * @return					true if the thread was successfully joined, false otherwise
 */
inline bool try_join_once(std::unique_ptr<std::thread> &thread)
{
    if (thread && thread->joinable())
    {
        thread->join();
        thread.reset();
        return true;
    }
    return false;
}

/**
 * @brief timed_join	Tries to join the passed thread until it succeeds or duration passes
 * @param thread		unique_ptr to a std::tread. Will be reset on success
 * @param duration		max duration to try joining
 * @return true on success, false otherwise
 */
inline bool timed_join(thread_p &thread, std::chrono::milliseconds duration = max_join_wait)
{
    const auto start = Clock::now();
    while (Clock::now() - start < duration)
    {
        if (try_join_once(thread))
            return true;
        std::this_thread::sleep_for(std::chrono::milliseconds(500));
    }
    return false;
}

/**
 * @brief timed_join_or_detach	Join the thread or detach it if not possible within specified
 * duration
 * @param thread				unique_ptr to a std::tread. Will be reset on success
 * @param duration max			duration to try joining
 */
inline void timed_join_or_detach(
    thread_p &thread, std::chrono::milliseconds duration = max_join_wait)
{
    if (!timed_join(thread, duration))
    {
        thread->detach();
        std::cerr << "Thread didn't join in time!" << std::endl;
    }
}

/**
 * @brief timed_join_or_detach	Join the thread or detach it if not possible within specified
 * duration
 * @param threads				list of unique_ptrs to std::threads. Guaranteed to be empty
 * afterwards.
 * @param duration				duration to try joining
 */
inline void timed_join_or_detach(
    std::list<thread_p> &threads, std::chrono::milliseconds duration = max_join_wait)
{
    const auto start = Clock::now();
    while (Clock::now() - start < duration && !threads.empty())
    {
        for (auto it = threads.begin(); it != threads.end();)
        {
            if (try_join_once(*it))
                it = threads.erase(it);
            else
                ++it;
        }
        std::this_thread::sleep_for(std::chrono::milliseconds(500));
    }
    if (!threads.empty())
    {
        std::cout << threads.size() << " stream threads still running!" << std::endl;
        for (auto &t : threads)
            t->detach();
        threads.clear();
    }
}

// CJS-2: Add setting information (applicationSettings) and chart window (streamDataChartWindowMap) of U-BCI program as arguments
recording::recording(const std::string &filename, const std::vector<lsl::stream_info> &streams,
                     const std::vector<std::string> &watchfor, std::map<std::string, int> syncOptions,
                     ApplicationSettings *applicationSettings, MainWindow *mainWindow,
                     std::map<std::string, StreamDataChartWindow *> &streamDataChartWindowMap,
                     bool collect_offsets)
    : file_(filename), offsets_enabled_(collect_offsets), unsorted_(false), streamid_(0),
      shutdown_(false), headers_to_finish_(0), streaming_to_finish_(0),
      sync_options_by_stream_(std::move(syncOptions)),
      m_applicationSettings(applicationSettings),
      m_mainWindow(mainWindow),
      m_streamDataChartWindowMap(streamDataChartWindowMap),
      m_numStreamsReady(0)
{
    // CJS-2: Start the data output module
    m_outputModuleManager = new OutputModuleManager(applicationSettings, mainWindow);

    // CJS-2: Start the online data process module
    m_onlineProcessingModuleManager = new OnlineProcessingModuleManager(
        applicationSettings->GetOnlineProcessingModuleSettingsMap(),
        applicationSettings->GetStreamDataSettingsMap(),
        m_outputModuleManager, mainWindow);

    // create a recording thread for each stream
    for (const auto &stream : streams)
        stream_threads_.emplace_back(
            new std::thread(&recording::record_from_streaminfo, this, stream, true));

    // create a resolve-and-record thread for each item in the watchlist
    for (const auto &thread : watchfor)
        stream_threads_.emplace_back(
            new std::thread(&recording::record_from_query_results, this, thread));

    // create a boundary chunk writer thread
    boundary_thread_ = std::make_unique<std::thread>(&recording::record_boundaries, this);
}

recording::~recording()
{
    try
    {
        qInfo() << "Stop the recordings...";

        // set the shutdown flag (from now on no more new streams)
        shutdown_ = true;

        // stop the threads
        timed_join_or_detach(stream_threads_, max_join_wait);
        if (!timed_join(boundary_thread_, max_join_wait + boundary_interval))
        {
            std::cout << "boundary_thread didn't finish in time!" << std::endl;
            boundary_thread_->detach();
        }
        std::cout << "Closing the file." << std::endl;

        // CJS-2:
        for (StreamDataPreprocessorManager *streamDataPreprocessorManager :
             m_streamDataPreprocessorManager)
            delete streamDataPreprocessorManager;
        delete m_onlineProcessingModuleManager;
        delete m_outputModuleManager;
        delete m_applicationSettings;
    }
    catch (std::exception &e)
    {
        std::cout << "Error while closing the recording: " << e.what() << std::endl;
    }
}

void recording::requestStop() noexcept
{
    shutdown_ = true;
}

void recording::record_from_query_results(const std::string &query)
{
    try
    {
        std::set<std::string> known_uids;       // set of previously seen stream uid's
        std::set<std::string> known_source_ids; // set of previously seen source id's
        std::list<thread_p> threads;            // our spawned threads
        std::cout << "Watching for a stream with properties " << query << std::endl;
        while (!shutdown_)
        {
            // periodically re-resolve the query
            std::vector<lsl::stream_info> results = lsl::resolve_stream(query, 0, resolve_interval);
            // for each result...
            for (auto &result : results)
            {
                // if it is a new stream...
                std::string _uid = result.uid();
                std::string _src_id = result.source_id();
                if (!known_uids.count(result.uid()))
                    // and doesn't have a previously seen source id...
                    if (!result.source_id().empty() &&
                        (!known_source_ids.count(result.source_id())))
                    {
                        std::cout << "Found a new stream named " << result.name()
                                  << ", adding it to the recording." << std::endl;
                        // start a new recording thread
                        threads.emplace_back(new std::thread(
                            &recording::record_from_streaminfo, this, result, false));
                        // ... and add it to the lists of known id's
                        known_uids.insert(result.uid());
                        if (!result.source_id().empty())
                            known_source_ids.insert(result.source_id());
                    }
            }
        }
        // wait for all our threads to join
        timed_join_or_detach(threads, max_join_wait);
    }
    catch (std::exception &e)
    {
        std::cout << "Error in the record_from_query_results thread: " << e.what() << std::endl;
    }
}

void recording::record_from_streaminfo(const lsl::stream_info &src, bool phase_locked)
{
    try
    {
        double first_timestamp, last_timestamp;
        uint64_t sample_count = 0;
        // obtain a fresh streamid
        streamid_t streamid = fresh_streamid();

        inlet_p in;
        lsl::stream_info info;

        // --- headers phase
        try
        {
            enter_headers_phase(phase_locked);

            // open an inlet to read from (and subscribe to data immediately)
            in.reset(new lsl::stream_inlet(src));
            auto it = sync_options_by_stream_.find(src.name() + " (" + src.hostname() + ")");
            if (it != sync_options_by_stream_.end())
                in->set_postprocessing(it->second);

            try
            {
                in->open_stream(max_open_wait);
                std::cout << "Opened the stream " << src.name() << "." << std::endl;
            }
            catch (lsl::timeout_error &)
            {
                std::cout
                    << "Subscribing to the stream " << src.name()
                    << " is taking relatively long; collection from this stream will be delayed."
                    << std::endl;
            }

            // retrieve the stream header & get its XML version
            info = in->info();
            file_.write_stream_header(streamid, info.as_xml());
            std::cout << "Received header for stream " << src.name() << "." << std::endl;

            leave_headers_phase(phase_locked);
        }
        catch (std::exception &)
        {
            leave_headers_phase(phase_locked);
            throw;
        }

        // --- streaming phase
        try
        {
            // this waits until we are done writing all headers for the initial set of
            // (phase-locked) streams (any streams that are discovered later, if any, will not wait)
            // we're doing this so that all headers of the initial set of streams come first, so the
            // XDF file is properly sorted unless we discover some streams later which someone
            // "forgot to turn on" before the recording started; in that case the file would have to
            // be post-processed to be in properly sorted (seekable) format
            enter_streaming_phase(phase_locked);
            std::cout << "Started data collection for stream " << src.name() << "." << std::endl;

            // now write the actual sample chunks...
            switch (src.channel_format())
            {
            case lsl::cf_int8:
                typed_transfer_loop<char>(streamid, info.nominal_srate(), in, first_timestamp,
                                          last_timestamp, sample_count);
                break;
            case lsl::cf_int16:
                typed_transfer_loop<int16_t>(streamid, info.nominal_srate(), in, first_timestamp,
                                             last_timestamp, sample_count);
                break;
            case lsl::cf_int32:
                typed_transfer_loop<int32_t>(streamid, info.nominal_srate(), in, first_timestamp,
                                             last_timestamp, sample_count);
                break;
            case lsl::cf_float32:
                typed_transfer_loop<float>(streamid, info.nominal_srate(), in, first_timestamp,
                                           last_timestamp, sample_count);
                break;
            case lsl::cf_double64:
                typed_transfer_loop<double>(streamid, info.nominal_srate(), in, first_timestamp,
                                            last_timestamp, sample_count);
                break;
            case lsl::cf_string:
                typed_transfer_loop<std::string>(streamid, info.nominal_srate(), in,
                                                 first_timestamp, last_timestamp, sample_count);
                break;
            default:
                // unsupported channel format
                throw std::runtime_error(
                    std::string("Unsupported channel format in stream ") += src.name());
            }

            leave_streaming_phase(phase_locked);
        }
        catch (std::exception &)
        {
            leave_streaming_phase(phase_locked);
            throw;
        }

        // --- footers phase
        try
        {
            enter_footers_phase(phase_locked);

            // now generate the [StreamFooter] contents
            std::ostringstream footer;
            footer.precision(16);
            // [Content]
            footer << "<?xml version=\"1.0\"?><info><first_timestamp>" << first_timestamp
                   << "</first_timestamp><last_timestamp>" << last_timestamp
                   << "</last_timestamp><sample_count>" << sample_count << "</sample_count>";
            footer << "<clock_offsets>";
            {
                // including the clock_offset list
                std::lock_guard<std::mutex> lock(offset_mut_);
                for (const auto pair : offset_lists_[streamid])
                {
                    footer << "<offset><time>" << pair.first << "</time><value>" << pair.second
                           << "</value></offset>";
                }
                footer << "</clock_offsets></info>";
            }
            file_.write_stream_footer(streamid, footer.str());

            std::cout << "Wrote footer for stream " << src.name() << "." << std::endl;
            leave_footers_phase(phase_locked);
        }
        catch (std::exception &)
        {
            leave_footers_phase(phase_locked);
            throw;
        }
    }
    catch (std::exception &e)
    {
        std::cout << "Error in the record_from_streaminfo thread: " << e.what() << std::endl;
    }
}

void recording::record_boundaries()
{
    try
    {
        auto next_boundary = Clock::now() + boundary_interval;
        while (!shutdown_)
        {
            std::this_thread::sleep_for(std::chrono::milliseconds(500));
            if (Clock::now() > next_boundary)
            {
                file_.write_boundary_chunk();
                next_boundary = Clock::now() + boundary_interval;
            }
        }
    }
    catch (std::exception &e)
    {
        std::cout << "Error in the record_boundaries thread: " << e.what() << std::endl;
    }
}

void recording::record_offsets(
    streamid_t streamid, const inlet_p &in, std::atomic<bool> &offset_shutdown) noexcept
{
    try
    {
        while (!shutdown_ && !offset_shutdown)
        {
            // sleep for the interval
            std::this_thread::sleep_for(offset_interval);
            // query the time offset
            double offset, now;
            try
            {
                offset = in->time_correction(2);
                now = lsl::local_clock();
            }
            catch (lsl::timeout_error &)
            {
                std::cerr << "Timeout in time correction query for stream " << streamid
                          << std::endl;
            }
            file_.write_stream_offset(streamid, now, offset);
            // also append to the offset lists
            std::lock_guard<std::mutex> lock(offset_mut_);
            offset_lists_[streamid].emplace_back(now - offset, offset);
        }
    }
    catch (std::exception &e)
    {
        std::cout << "Error in the record_offsets thread: " << e.what() << std::endl;
    }
    std::cout << "Offsets thread is finished" << std::endl;
}

void recording::enter_headers_phase(bool phase_locked)
{
    if (phase_locked)
    {
        std::lock_guard<std::mutex> lock(phase_mut_);
        headers_to_finish_++;
    }
}

void recording::leave_headers_phase(bool phase_locked)
{
    if (phase_locked)
    {
        std::unique_lock<std::mutex> lock(phase_mut_);
        headers_to_finish_--;
        lock.unlock();
        ready_for_streaming_.notify_all();
    }
}

void recording::enter_streaming_phase(bool phase_locked)
{
    if (phase_locked)
    {
        std::unique_lock<std::mutex> lock(phase_mut_);
        ready_for_streaming_.wait_for(
            lock, max_headers_wait, [this]()
            { return this->ready_for_streaming(); });
        streaming_to_finish_++;
    }
}

void recording::leave_streaming_phase(bool phase_locked)
{
    if (phase_locked)
    {
        std::unique_lock<std::mutex> lock(phase_mut_);
        streaming_to_finish_--;
        lock.unlock();
        ready_for_footers_.notify_all();
    }
}

void recording::enter_footers_phase(bool phase_locked)
{
    if (phase_locked)
    {
        std::unique_lock<std::mutex> lock(phase_mut_);
        ready_for_footers_.wait_for(
            lock, max_footers_wait, [this]()
            { return this->ready_for_footers(); });
    }
}

template <class T>
void recording::typed_transfer_loop(streamid_t streamid, double srate, const inlet_p &in,
                                    double &first_timestamp, double &last_timestamp, uint64_t &sample_count)
{
    // optionally start an offset collection thread for this stream
    std::atomic<bool> offset_shutdown{false};
    thread_p offset_thread(offsets_enabled_ ? new std::thread(&recording::record_offsets, this,
                                                              streamid, in, std::ref(offset_shutdown))
                                            : nullptr);
    try
    {
        // temporary data
        std::vector<T> chunk;
        std::vector<double> timestamps;

        double sample_interval = srate ? 1.0 / srate : 0;

        // ===== CJS-2 =====
        // Update the settings of stream data based on information inside stream data
        StreamDataSettings *streamDataSettings = m_applicationSettings->GetStreamDataSettings(
            QString::fromStdString(in->info().name()));
        streamDataSettings->SetStreamDataParams(in->info());

        // Generate the waiting array to send stream data to the data preprocessing object
        std::queue<StreamData *> streamDataQueue;

        // Start the preprocessing thread of stream data
        LaunchPreprocessingThread(streamDataSettings, streamDataQueue, in->info().name(), m_mainWindow);

        // Set the necessary parameter
        int bufferSize = streamDataSettings->GetBufferSize();
        int usedChannelCount = streamDataSettings->GetUsedChannelCount();
        int channelCount = streamDataSettings->GetChannelCount();
        std::vector<int> usedChNumList = streamDataSettings->GetUsedChannelNumList();

        // Create the buffer to store each time-series data and time data of stream data
        std::vector<double> timeSeriesDataBuffer;
        timeSeriesDataBuffer.reserve(bufferSize * usedChannelCount);
        std::vector<double> timestampDataBuffer;
        timestampDataBuffer.reserve(bufferSize);
        int bufferCount = 0;
        // =================

        // Pull the first sample
        first_timestamp = 0.0;
        while (!shutdown_ && first_timestamp == 0.0)
            first_timestamp = last_timestamp = in->pull_sample(chunk, 4.0);
        if (!shutdown_)
        {
            timestamps.push_back(first_timestamp);
            file_.write_data_chunk(streamid, timestamps, chunk, (uint32_t)in->get_channel_count());
        }

        // debug: Calculate the sampling rate
        size_t debugSampleCount = 0;
        QElapsedTimer timer;
        timer.start();

        auto next_pull = Clock::now();
        while (!shutdown_)
        {
            // get a chunk from the stream
            in->pull_chunk_multiplexed(chunk, &timestamps, 1e-6);

            // ===== CJS-2 =====
            // Store the stream data to buffer and send the data preprocessing object when the data exists in the chunk of stream data
            // This program replaces the time data by zero in certain condition after process in side Lab Recorder.
            // To avoid using the time data after processing, the process is executed before the proces.
            if (chunk.size() > 0)
                ReceiveStreamData(chunk, timestamps, in, streamDataSettings->GetDataType(),
                                  channelCount, usedChannelCount, usedChNumList, timeSeriesDataBuffer,
                                  timestampDataBuffer, bufferSize, bufferCount, streamDataQueue);
            // =================

            // for each sample...
            for (double &ts : timestamps)
            {
                // if the time stamp can be deduced from the previous one...
                if (last_timestamp + sample_interval == ts)
                {
                    last_timestamp = ts + sample_interval;
                    ts = 0;
                }
                else
                    last_timestamp = ts;
            }

            // write the actual chunk
            file_.write_data_chunk(streamid, timestamps, chunk, in->get_channel_count());

            sample_count += timestamps.size();

            // debug: Calculate the sampling rate
            debugSampleCount += timestamps.size();

            next_pull += chunk_interval;
            //std::this_thread::sleep_until(next_pull);

            // CJS-2: It reduces lop waiting time drastically because U-BCI process the job by small steps
            std::this_thread::sleep_for(std::chrono::milliseconds(10));
        }

        // debug: Show the sampling rate
        qint64 elapsedTime = timer.elapsed();
        qDebug() << streamDataSettings->GetStreamDataName() << ": (recording)"
                 << "Elapsed time:" << elapsedTime << ", Sample count:" << debugSampleCount
                 << ", Sampling rate (kHz): " << (double)debugSampleCount / elapsedTime;
    }
    catch (std::exception &e)
    {
        std::cerr << "Error in transfer thread: " << e.what() << std::endl;
        offset_shutdown = true;
        timed_join_or_detach(offset_thread);
        throw;
    }

    timed_join_or_detach(offset_thread);
}

// CJS-2: Start the preprocessing thread of stream data
void recording::LaunchPreprocessingThread(StreamDataSettings *streamDataSettings,
                                          std::queue<StreamData *> &streamDataQueue,
                                          const std::string &streamDataName,
                                          MainWindow *mainWindow)
{
    // Generate the preprocessing thread of stream data
    m_streamDataPreprocessorManager.emplace_back(new StreamDataPreprocessorManager(
        streamDataSettings,
        streamDataQueue,
        m_streamDataChartWindowMap[streamDataName],
        m_onlineProcessingModuleManager,
        &shutdown_,
        mainWindow));

    // Update the preprocessed thread number that is ready
    ++m_numStreamsReady;

    // Wait until all the stream data's preprocessing thread has been generated
    while (m_numStreamsReady < m_applicationSettings->GetNumStreams())
        std::this_thread::sleep_for(std::chrono::milliseconds(100));
}

// CJS-2: Store the stream data to buffer and send the data preprocessing object when the data exists in the chunk of stream data
// Chunk time-series data of stream data has a data type of std::vector<T>.
// Timestamps time data stores the acquired time of each sample by a data type of std::vector<double>.
template <class T>
void recording::ReceiveStreamData(std::vector<T> &chunk,
                                  const std::vector<double> &timestamps,
                                  const inlet_p &in,
                                  const lsl::channel_format_t &dataType,
                                  const int channelCount,
                                  const int usedChannelCount,
                                  const std::vector<int> &usedChNumList,
                                  std::vector<double> &timeSeriesDataBuffer,
                                  std::vector<double> &timestampDataBuffer,
                                  const int bufferSize, int &bufferCount,
                                  std::queue<StreamData *> &streamDataQueue)
{
    // Convert the chunk data to double data type.
    int sampleSize = static_cast<int>(timestamps.size());
    std::vector<double> timeSeriesData;
    timeSeriesData.reserve(sampleSize * usedChannelCount);
    switch (dataType)
    {
    case lsl::cf_float32:
        for (int sample = 0; sample < sampleSize; sample++)
        {
            for (int i = 0; i < usedChannelCount; i++)
            {
                float *pSample = reinterpret_cast<float *>(
                    &chunk[channelCount * sample + usedChNumList[i]]);
                timeSeriesData.emplace_back(static_cast<double>(*pSample));
            }
        }
        break;
    case lsl::cf_double64:
        for (int sample = 0; sample < sampleSize; sample++)
        {
            for (int i = 0; i < usedChannelCount; i++)
            {
                double *pSample = reinterpret_cast<double *>(
                    &chunk[channelCount * sample + usedChNumList[i]]);
                timeSeriesData.emplace_back(*pSample);
            }
        }
        break;
    case lsl::cf_string:
    {
        // CJS-2: Show the string to the main window's log display
        std::string *str = reinterpret_cast<std::string *>(&chunk[0]);
        m_mainWindow->WriteLog(QString::fromStdString(*str));
    }
        return;
    default:
        // Do nothing to data type that is not yet compatible
        return;
        //        // Output error to data type that is not yet compatible
        //        throw std::runtime_error(
        //            std::string("Unsupported channel format in stream ") += in->info().name());
    }

    // Correct the time data
    std::vector<double> timestampData;
    double timeCorrection = in->time_correction();
    for (double timestamp : timestamps)
        timestampData.emplace_back(timestamp + timeCorrection);

    // Store the time-series data and time data to the buffer
    BufferStreamData(timeSeriesData, timestampData, sampleSize, usedChannelCount,
                     bufferSize, bufferCount, timeSeriesDataBuffer,
                     timestampDataBuffer, streamDataQueue);
}

// CJS-2: Store the time-series data and time data to the buffer and send the data toe the preprocess object when the buffer is full
void recording::BufferStreamData(const std::vector<double> &timeSeriesData,
                                 const std::vector<double> &timestampData,
                                 const int sampleSize, const int usedChannelCount,
                                 const int bufferSize, int &bufferCount,
                                 std::vector<double> &timeSeriesDataBuffer,
                                 std::vector<double> &timestampDataBuffer,
                                 std::queue<StreamData *> &streamDataQueue)
{
    // Set the necessary parameter
    int startItr = 0;                                         // Starting iteration of empty area of buffer
    int remainings = sampleSize - (bufferSize - bufferCount); // Sample number that cannot be stored to the buffer
    if (remainings < 0)
        remainings = 0;
    int currentSamplesStored = 0; // Sample number that will be stored to the buffer in one process
    int totalSamplesStored = 0;   // Total number of sample that is store to the buffer until now

    // Process until all the time-series data and time data have been stored to the buffer
    do
    {
        // Store the time-series data and time data to the buffer
        std::copy(timeSeriesData.begin() + startItr * usedChannelCount,
                  timeSeriesData.end() - remainings * usedChannelCount,
                  back_inserter(timeSeriesDataBuffer));
        std::copy(timestampData.begin() + startItr, timestampData.end() - remainings,
                  back_inserter(timestampDataBuffer));

        // Update the stored sample number
        currentSamplesStored = sampleSize - remainings - startItr;
        totalSamplesStored += currentSamplesStored;
        bufferCount += currentSamplesStored;

        // When the buffer is full
        if (bufferCount >= bufferSize)
        {
            // Take the copy of buffer (time-series data and time data)
            std::vector<double> *timeSeriesDataCopy = new std::vector<double>;
            std::copy(timeSeriesDataBuffer.begin(), timeSeriesDataBuffer.end(),
                      back_inserter(*timeSeriesDataCopy));
            std::vector<double> *timestampDataCopy = new std::vector<double>;
            std::copy(timestampDataBuffer.begin(), timestampDataBuffer.end(),
                      back_inserter(*timestampDataCopy));

            // Store the copy of buffer to the waiting array and send the data preprocessing object
            streamDataQueue.push(new StreamData(timeSeriesDataCopy, timestampDataCopy));

            // Empty the buffer
            timeSeriesDataBuffer.clear();
            timestampDataBuffer.clear();
            bufferCount = 0;
        }

        // Set the parameter for the next process
        startItr += currentSamplesStored;
        remainings -= (bufferSize - bufferCount);
        if (remainings < 0)
            remainings = 0;
    } while (totalSamplesStored < sampleSize);
}
