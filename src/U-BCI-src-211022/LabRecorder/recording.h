...
#include "StreamData/StreamDataPreprocessorManager.h"
...
private:
    ...
    void leave_footers_phase(bool)
    { /* Nothing to do. Ignore warning. */
    }
    ...
    void LaunchPreprocessingThread(StreamDataSettings *streamDataSettings,
    ...
    template <class T>
    void ReceiveStreamData(std::vector<T> &chunk,
    ...
    // Store the time-series data and time data to the buffer and send the data toe the preprocess object when the buffer is full
    void BufferStreamData(const std::vector<double> &timeSeriesData,
    ...
    // Holds the settings information that relates to the U-BCI program
    ApplicationSettings *m_applicationSettings;

    // Main window
    MainWindow *m_mainWindow;

    // Chart window corresponds to each stream data
    // Manage the name  of stream data
    std::map<std::string, StreamDataChartWindow *> m_streamDataChartWindowMap;

    // Preprocessing thread for each stream data
    std::vector<StreamDataPreprocessorManager *> m_streamDataPreprocessorManager;

    // Preprocessing thread number of prepared stream data
    std::atomic<int> m_numStreamsReady;

    // Manage the online processing module
    OnlineProcessingModuleManager *m_onlineProcessingModuleManager;

    // Manage the data output module
    OutputModuleManager *m_outputModuleManager;
};
