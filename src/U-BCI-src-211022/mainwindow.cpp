...
// recording class
#include "LabRecorder/recording.h"
#include "LabRecorder/tcpinterface.h"
...
void MainWindow::load_config(QString filename)
{
    ...
    try
    {
        // CJS-2: Save the setting file after modifing the back slash if back slash exists
        Utility::CheckPathSeparator(QDir::cleanPath(filename));
        ...
        for (QString &oss : onlineSyncStreams)
        {
            ...
            {
                if (word == "post_clocksync")
                {
                    val |= lsl::post_clocksync;
                }
                if (word == "post_dejitter")
                {
                    val |= lsl::post_dejitter;
                }
                if (word == "post_monotonize")
                {
                    val |= lsl::post_monotonize;
                }
                if (word == "post_threadsafe")
                {
                    val |= lsl::post_threadsafe;
                }
                if (word == "post_ALL")
                {
                    val = lsl::post_ALL;
        ...
        if (pt.contains("SessionBlocks"))
        {
            taskNames = pt.value("SessionBlocks").toStringList();
        }
        ...
        if (pt.contains("StudyRoot"))
        {
            studyRoot = pt.value("StudyRoot").toString();
        }
        if (pt.contains("PathTemplate"))
        {
            legacyTemplate = pt.value("PathTemplate").toString();
        }
        ...
        if (recFilename.contains(counterPlaceholder()))
        {
            for (int i = 1; i < 1001; i++)
            {
                ...
                if (!QFileInfo::exists(replaceFilename(recFilename)))
                    break;
            }
        }
        ...
        // CJS-2: Member variables holds the setting file's path
...
void MainWindow::save_config(QString filename)
{
    ...
    for (auto &k : knownStreams)
    {
        if (k.checked)
        {
            requiredStreams.append(k.listName());
        }
    }
...
std::vector<lsl::stream_info> MainWindow::refreshStreams()
{
    ...
    for (auto &s : resolvedStreams)
    {
        ...
        if (!known)
        {
            ...
            if (found)
            {
                missingStreams.remove(info_to_listName(s));
            }
        }
    }
    ...
    for (auto &k : knownStreams)
    {
        ...
        {
            bool checked = false;
            for (auto &fi : foundItems)
            {
                checked |= fi->checkState() == Qt::Checked;
            }
            k.checked = checked;
        }
    }
    ...
    for (auto &r : resolvedStreams)
    {
        for (auto &k : knownStreams)
        {
            if ((r.name() == k.name) && (r.type() == k.type) && (r.source_id() == k.id))
            {
                if (k.checked)
                {
                    requestedAndAvailableStreams.push_back(r);
                }
                break;
            }
        }
...
void MainWindow::startRecording()
{
    ...
    if (!currentRecording)
    {
        ...
        if (recFileInfo.exists())
        {
            ...
            int i = 1;
            while (QFileInfo::exists(rename_to.arg(i)))
                i++;
        ...
        // CJS-2: Register each setting information of U-BCI based on the setting data read by Lab Recorder's configuration file
        ApplicationSettings *applicationSettings = new ApplicationSettings;
        try
        {
            RegisterUbciSettings(applicationSettings, m_settingsFilePath,
                                 requestedAndAvailableStreams);
        }
        catch (UbciException &e)
        {
            Utility::ShowWarningDialog("Improper U-BCI setting", e.what());
            return;
        }
        catch (std::exception &e)
        {
            Utility::ShowErrorDialog("Error in loading U-BCI settings", e.what());
            return;
        }

        // CJS-2: Generation the chart window of each stream data
        for (const auto &stream : requestedAndAvailableStreams)
        {
            const QString streamDataName = QString::fromStdString(stream.name());
            if (applicationSettings->GetStreamDataSettings(streamDataName)->IsChartDisplayed())
            {
                StreamDataSettings *streamDataSettings = applicationSettings->GetStreamDataSettings(streamDataName);
                m_streamDataChartWindowMap[stream.name()] = new StreamDataChartWindow(this, streamDataSettings);
            }
        }

        // CJS-2 modification: start point of stream data process
        currentRecording = std::make_unique<recording>(recFilename.toStdString(),
                                                       requestedAndAvailableStreams,
                                                       watchfor, syncOptionsByStreamName,
                                                       applicationSettings, this,
                                                       m_streamDataChartWindowMap, true);
...
void MainWindow::stopRecording()
{
    ...
    else
    {
        ...
        // CJS-2: Close the chart window
        for (auto itr = m_streamDataChartWindowMap.begin();
             itr != m_streamDataChartWindowMap.end(); itr++)
        {
            if (itr->second != nullptr)
                // Destroy the object when the screen has been closed
                itr->second->close();
        }
    }
}
...
void MainWindow::StopRecording(QString title, QString message)
{
    // Show error dialog
    Utility::ShowErrorDialog(title, message);

    // Error will occurr if the main screen is not in the front
    setWindowFlags(windowFlags() | Qt::WindowStaysOnTopHint);
    setWindowFlags(windowFlags() & ~Qt::WindowStaysOnTopHint);
    show();

    // Press the [Stop] button by program
...
void MainWindow::buildBidsTemplate()
{
    ...
    if (ui->lineEdit_participant->text().isEmpty())
    {
        ui->lineEdit_participant->setText("P001");
    }
    if (ui->lineEdit_session->text().isEmpty())
    {
        ui->lineEdit_session->setText("S001");
    }
    ...
    if (!ui->lineEdit_acq->text().isEmpty())
    {
        fname.append("_acq-%a");
    }
...
void MainWindow::buildFilename()
{
    ...
    if (tpl.contains(counterPlaceholder()))
    {
        ...
        {
            ui->spin_counter->setValue(i);
            if (!QFileInfo::exists(replaceFilename(tpl)))
                break;
        }
    }
...
// CJS-2: Register each setting information of U-BCI based on the setting data read by Lab Recorder's configuration file
void MainWindow::RegisterUbciSettings(ApplicationSettings *applicationSettings,
                                      const QString &settingsFilePath,
                                      const std::vector<lsl::stream_info> &requestedAndAvailableStreams)
{
    // Set the using LSL stream number
    applicationSettings->SetNumStreams(requestedAndAvailableStreams.size());

    // Read the setting data from the Lab Recorder's configuration file
    QSettings settings(QDir::cleanPath(settingsFilePath), QSettings::Format::IniFormat);

    // Register the setting of general program in U-BCI form setting information
    applicationSettings->RegisterUbciSettings(settings);

    // Get the setting information of each stream data and hold the object that manages the setting information of U-BCI
    for (const auto &stream : requestedAndAvailableStreams)
        applicationSettings->RegisterStreamDataSettings(
            QString::fromStdString(stream.name()), settings);

    // Get the setting information of each online process module and hold the object that manages the setting information of U-BCI
    QStringList moduleNameList = applicationSettings->GetProcessingModuleNameList();
    for (int i = 0; i < moduleNameList.size(); i++)
        applicationSettings->RegisterOnlineProcessingModuleSettings(moduleNameList[i], settings);

    // Register the setting information of the data output module
    applicationSettings->RegisterOutputModuleSettings(settings);

    // Confirm the validity of setting information
    applicationSettings->CheckUbciSettings();
...
void MainWindow::DisableStreamDataChartWindow(const QString &qWindowName)
{
    std::string windowName = qWindowName.toStdString();
    if (m_streamDataChartWindowMap.count(windowName) > 0)
        m_streamDataChartWindowMap[windowName] = nullptr;
}

// CJS-2: Add the log to the log display in the main window
void MainWindow::WriteLog(const QString &log)
{
    ui->_logConsole->append(log);
}
