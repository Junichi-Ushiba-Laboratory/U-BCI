...
// LSL API
//#include <lsl_cpp.h>
#include "../../LSL/liblsl-1.14.0-Win_amd64/include/lsl_cpp.h"
...
EGIAmpWindow::EGIAmpWindow(QWidget* parent, const std::string& config_file)
    ...
    // for Physio16
    connect(this, &EGIAmpWindow::fieldsEnabled, this->ui->physioPort1CheckBox, &QCheckBox::setEnabled);
    connect(this, &EGIAmpWindow::fieldsEnabled, this->ui->PhysioPort2CheckBox, &QCheckBox::setEnabled);

    // keyboard shortcut
    QShortcut *linkButtonShortcut = new QShortcut(QKeySequence("L"), ui->linkButton);
    connect(linkButtonShortcut, &QShortcut::activated, this, &EGIAmpWindow::link_ampserver);
}
...
void EGIAmpWindow::load_config(const std::string &filename) {
	...
	try {
        ...
        // for Physio16
        ui->physioPort1CheckBox->setChecked(pt.get<bool>("settings.physio1", false));
        ui->PhysioPort2CheckBox->setChecked(pt.get<bool>("settings.physio2", false));
    } catch (std::exception&) {
...
void EGIAmpWindow::save_config(const std::string& filename) {
	...
	try {
        ...
        // for Physio16
        pt.put("settings.physio1", ui->physioPort1CheckBox->isChecked());
        pt.put("settings.physio2", ui->PhysioPort2CheckBox->isChecked());
	} catch (std::exception& e) {
...
void EGIAmpWindow::unlockUI() {
    Q_EMIT this->setLinkButtonText("Link (L)");
    Q_EMIT this->fieldsEnabled(true);
...
void EGIAmpWindow::lockUI() {
    Q_EMIT this->setLinkButtonText("Unlink (L)");
    Q_EMIT this->fieldsEnabled(false);
}
...
void EGIAmpWindow::link_ampserver() {
	if (reader_thread_) {
		...
		try {
            ...
            Q_EMIT this->enableSampleRateSelection(true);

		} catch (std::exception& e) {
            Q_EMIT this->error(QStringLiteral("Could not stop the background processing: ") +
			                 e.what());
			return;
		}

		// indicate that we are now successfully unlinked
        ui->linkButton->setText("Link (L)   ");
        ...
        // for Physio16 port
        is_connected_physio_1_ = ui->physioPort1CheckBox->isChecked();
        is_connected_physio_2_ = ui->PhysioPort2CheckBox->isChecked();
		...
		try {
			...
			if (this->getAmplifierDetails(amplifierId)) {
                ...
                Q_EMIT this->enableSampleRateSelection(false);
				...
				if (this->channelCount_ != 0) {
                    ...
                    Q_EMIT this->channelCountUpdated(nChannels);
				}
				...
				reader_thread_ = std::make_unique<std::thread>(
				    [this, address, amplifierId, sampleRate, nChannels]() {
                        Q_EMIT this->appendStatusMessage("Amplifier initialized.\n");
					    try {
                            ...
                            Q_EMIT this->error(QStringLiteral("Encountered the following error "
            ...
            Q_EMIT this->error("Could not initialize the AmpServer interface due to a "
            ...
            Q_EMIT this->error(QStringLiteral("Could not initialize the AmpServer interface: ") +
			                 e.what());
			if (connected) halt_ampserver(amplifierId);
			return;
		}
		...
		// done, all successful
        // ui->linkButton->setText("Unlink (L)");
        Q_EMIT this->disableUI();
	}
}
...
bool EGIAmpWindow::initAmplifier(bool verbose) {
    ...
    Q_EMIT this->appendStatusMessage("Initializing Amplifier...\n");
...
//	    "cmd_SetDecimatedRate", std::to_string(amplifierId), "0", sampleRate.toStdString());
    ...
    // get Physio16 connection
    std::string physioConnectionResponse =
        this->sendCommand("cmd_GetPhysioConnectionStatus", std::to_string(amplifierId), "0", "0");
	...
	if (verbose) {
        Q_EMIT this->appendStatusMessage("Stop: " + QString::fromStdString(stopResponse.c_str()));
        Q_EMIT this->appendStatusMessage("SetPower: " +
		                               QString::fromStdString(setPowerOffResponse.c_str()));
        Q_EMIT this->appendStatusMessage("SetDecimatedRate: " +
		                               QString::fromStdString(setSampleRateResponse.c_str()));
        Q_EMIT this->appendStatusMessage("SetPower: " +
		                               QString::fromStdString(setPowerOnResponse.c_str()));
        Q_EMIT this->appendStatusMessage("Start: " + QString::fromStdString(startResponse.c_str()));
        Q_EMIT this->appendStatusMessage("PhysioConnectionStatus: " +
                                       QString::fromStdString(physioConnectionResponse.c_str()));
        Q_EMIT this->appendStatusMessage("DefaultAcquisitionState: " +
...
bool EGIAmpWindow::getAmplifierDetails(int amplifierId) {
	try {
        ...
        Q_EMIT this->appendStatusMessage(
		...
		while (std::regex_search(details, match, token)) {
			...
			if (key.find("packet_format") != std::string::npos) {
                ...
                Q_EMIT this->appendStatusMessage("    Packet Format: " +
                ...
                Q_EMIT this->appendStatusMessage("    Channel Count: " +
				                               QString::fromStdString(value));
                ...
                Q_EMIT this->appendStatusMessage("    Serial Number: " +
				                               QString::fromStdString(value));
			} else if (key.find("system_version") != std::string::npos) {

				// Print the firmware version.
                Q_EMIT this->appendStatusMessage("    Firmware Version: " +
				                               QString::fromStdString(value));
			}
			details = match.suffix().str();
		}
		if (this->amplifierType_ == NA300) {
            Q_EMIT this->appendStatusMessage("    Amplifier Type: Net Amps 300");
		} else if (this->amplifierType_ == NA400) {
            Q_EMIT this->appendStatusMessage("    Amplifier Type: Net Amps 400");
		} else if (this->amplifierType_ == NA410) {
            Q_EMIT this->appendStatusMessage("    Amplifier Type: Net Amps 410");
		}
        Q_EMIT this->appendStatusMessage("__________________________\n__________________________\n");
		if (this->amplifierType_ == NA300) {
...
void EGIAmpWindow::read_packet_format_2(int numChannels, int* packetsReceived) {
	...
	dataStream_.clear();
    Q_EMIT this->appendStatusMessage("Starting stream.\n");
	while (dataStream_.good() && !stop_flag) {
		...
		for (int s = 0; s < nSamples && dataStream_.good(); s++) {
			...
			if (!outlet) {
                ...
                Q_EMIT this->channelCountUpdated(nChannels);
                Q_EMIT this->channelCountEnabled(false);

				// create streaminfo
				std::string streamname = "EGI NetAmp " + std::to_string(header.ampID);

                // for Physio16
                int nChannelsTotal = nChannels;
                if (is_connected_physio_1_) {
                    nChannelsTotal += 16;
                }
                if (is_connected_physio_2_) {
                    nChannelsTotal += 16;
                }

                lsl::stream_info info(streamname, "EEG", nChannelsTotal,
			...
			if (lastPacketCounter == 0 && !firstPacketReceived) {
                Q_EMIT this->appendStatusMessage("Stream Started.\n");
			}
			...
			if (lastTimeStamp_ != 0 &&
                ...
                Q_EMIT this->appendStatusMessage("Time Stamp: " +
            ...
            if (is_connected_physio_1_) {
                for (int ichannel = 0; ichannel < 16; ichannel++) {
                    if (ichannel <= 7) {
                        samples.push_back(static_cast<float>(packet.pib1_Data[ichannel]) *
                                          this->scalingFactor_physio_07);
                    }
                    else {
                        samples.push_back(static_cast<float>(packet.pib1_Data[ichannel]) *
                                          this->scalingFactor_physio_815);
                    }
                }
            }
            if (is_connected_physio_2_) {
                for (int ichannel = 0; ichannel < 16; ichannel++) {
                    if (ichannel <= 7) {
                        samples.push_back(static_cast<float>(packet.pib2_Data[ichannel]) *
                                          this->scalingFactor_physio_07);
                    }
                    else {
                        samples.push_back(static_cast<float>(packet.pib2_Data[ichannel]) *
    ...
    if (!dataStream_.good() && !stop_flag) { Q_EMIT this->error("The stream was lost."); }
    Q_EMIT this->enableUI();
}
...
void EGIAmpWindow::halt_ampserver(int id) {
    Q_EMIT this->appendStatusMessage("Stopping stream...\n");
	try {
        ...
        Q_EMIT this->appendStatusMessage("Stream Stopped.\n");
	} catch (...) {}
	stop_flag = false;
}
